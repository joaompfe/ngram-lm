...
cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_DEPENDS_USE_COMPILER=FALSE -G "CodeBlocks - Unix Makefiles" $PROJECT_DIR -B "${CMAKE_BUILD_DIR}"
...
# TODO use other test framework that works with pypy
for PYBIN in /opt/python/*/bin/; do
    "${PYBIN}/python" -m pip install pytest
    "${PYBIN}/pip" install ngram-lm --no-index -f "${REPAIRED_WHEELS_HOUSE}"
    (cd "$PROJECT_DIR"; "${PYBIN}/pytest" src/python)
done